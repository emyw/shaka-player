...
// const base64ToArrayBuffer2 = (base64) => {
...
//     return r;
//   };
//   // const binaryString = atob(base64); // sometimes gets invalid character error (newlines?)
...
const initPlayer = async () => {
  ...
  //   });
  // }
  // const customShakaRequest = (type, provider) => {
  // origShakaRequest = shaka.net.HttpFetchPlugin.request_;
  ...
  //   if (state.currentProvider === "YOUTUBE" && requestType == shaka.net.NetworkingEngine.RequestType.SEGMENT) {
  //     let lastTime = Date.now();
  //     let range = "";
  //     options.headers = shaka.net.HttpFetchPlugin.headersToGenericObject_(options.headers);
  //     ({ range } = options.headers);
  //     range = (range.split('=') || ['', '0-1780'])[1];
  //     const start = Number(range.split('-')[0]);
  //     if (start < 2000) uri += `&range=0-${range.split('-')[1]}`;
  //     else uri += `&range=${range}`;
  ...
  //     headersReceived(proxiedRes.headers);
  //     const currentTime = Date.now();
  //     progressUpdated(currentTime - lastTime, 0, 0);
  //     // console.log(proxiedRes);
  //     // shaka.net.HttpFetchPlugin.request_ = origShakaRequest;
  ...
  //   return origShakaRequest(uri, requestType, options, abortStatus, progressUpdated, headersReceived, streamDataCallback);
  // };
  // };
  ...
  player.getNetworkingEngine().registerRequestFilter((type, request) => {
    ...
    if (type == shaka.net.NetworkingEngine.RequestType.SEGMENT) {
      ...
      if (["DISNEYPLUS"].includes(state.currentProvider) && request.uris && request.uris.length > 0) request.uris[0] += `?vary=${generateDeviceId(6)}`;
      if (state.currentProvider === "YOUTUBE") {
        ...
        uri += `&range=${range}`;
        request.uris[0] = uri;
        delete request.headers.Range; // remove this or else we'll get a preflight request that CORS will block
      }
  ...
  setInterval(() => {
    if (video.buffered.length > 0) {
      ...
      if (!video.paused && buffered < 0.5) {
        ...
        video.pause();
      } else if (pausedByBuffering && buffered >= Math.min(remainingTime, 10)) {
        console.log("video has buffered enough to continue...", Math.min(remainingTime, 10));
  ...
  try {
    ...
    player.configure({
      drm: {
        ...
        advanced: {
          'com.widevine.alpha': {
            serverCertificate: provider === "NETFLIX"
              ? base64ToArrayBuffer(NETFLIX_SERVER_CERT)
              : provider === "YOUTUBE"
                ? new Uint8Array(YOUTUBE_SERVER_CERT.split(','))
                : new Uint8Array(0),
      ...
      // },
      restrictions: { maxPixels: 852 * 480 },
      // restrictions: { minPixels: 1920 * 1080 },
